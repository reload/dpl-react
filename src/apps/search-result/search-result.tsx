...
const SearchResult: React.FC<SearchResultProps> = ({ q, pageSize }) => {
  const { filters, clearFilter, addFilterFromUrlParam } = useFilterHandler();
  ...
  useEffect(() => {
    addFilterFromUrlParam(FacetField.MaterialTypes);
    addFilterFromUrlParam(FacetField.WorkTypes);
