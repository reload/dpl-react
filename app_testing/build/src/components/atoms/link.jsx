...
const Link = ({ href, children, isNewTab, className, id, trackClick, dataCy }) => {
    ...
    if (!trackClick) {
        return (<a id={id} data-cy={id} href={String(href)} target={isNewTab ? "_blank" : ""} rel="noreferrer" className={className}>
        {children}
      </a>);
    }
    return (<span id={id} data-cy={dataCy || id} role="button" tabIndex={0} onClick={() => {
            trackClick().then(() => {
