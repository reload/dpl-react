...
const ButtonTag = ({ onClick, selected, children, size, removable = false, dataCy }) => {
    ...
    return (<button type="button" aria-pressed={selected ?? undefined} className={className} onClick={onClick} data-cy={dataCy}>
      {children}
      {removable && (<img className="tag-icon" src={icon_cross_svg_1.default} alt="close icon"/>)}
    </button>);
};
